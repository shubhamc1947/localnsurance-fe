...
import { CheckCircle, Users, UserX, ArrowLeft } from "lucide-react";

const StepIncludeSelf = () => {
  const { data, updateData, setCurrentStep } = useQuote();
  const totalMembers = data.ageGroups.reduce((s, g) => s + g.count, 0);
  const [selected, setSelected] = useState<boolean | null>(data.includesSelf);

  const handleContinue = () => {
    if (selected === null) return;
    updateData({ includesSelf: selected });
      ...
      <div className="grid grid-cols-2 gap-4 w-full max-w-lg mb-10">
        <button
          ...
          className={`p-6 rounded-xl border-2 transition-all text-left ${
            selected === true ? "border-accent bg-accent/5" : "border-border hover:border-muted-foreground/40"
          }`}
        >
        ...
        <button
          ...
          className={`p-6 rounded-xl border-2 transition-all text-left ${
            selected === false ? "border-accent bg-accent/5" : "border-border hover:border-muted-foreground/40"
          }`}
        >
...
export default StepIncludeSelf;
